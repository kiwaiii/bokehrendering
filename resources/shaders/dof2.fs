...
void main()
{
//	if(textureSize(PositionTex,0).x < 10000)
//	{
...
//	}
	...
	float lvl	= log2(2*r);
//	vec3 color	= textureLod(InputTex,pix,lvl).xyz;
//	vec3 color	= textureLod(InputTex,pix,lvl).xyz;
	vec3 ccenter= textureLod(InputTex,pix,0).xyz;;
	vec3 cneighs= textureLod(InputTex,pix,lvl).xyz;
...
//	cneighs/= (count==1?1:count-1);
	...
	if(pix.x<-10000)
	{
		float value = NearStart + NearEnd + FarStart + FarEnd + MaxRadius + float(nSamples) + ViewMat[0].x + IntThreshold + CoCThreshold + AreaFactor + Halton[1].x + textureLod(RotationTex,vec2(0,0),0).x;
