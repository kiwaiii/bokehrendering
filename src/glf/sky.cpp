...
namespace glf
{
	...
	//-------------------------------------------------------------------------
	void SkyBuilder::Update(bool _drawSun)
	{
	...
	//-------------------------------------------------------------------------
	NightSkyBuilder::~NightSkyBuilder()
	{
	...
	//-------------------------------------------------------------------------
	void NightSkyBuilder::SetTurbidity(		float _turbidity)
	{
		turbidity 		= _turbidity;
		sunIntensity 	= ComputeSunIntensity(sunTheta,sunPhi, turbidity);
		glProgramUniform1f(sky.program.id, sky.turbidityVar, turbidity);
...
//glPointSize(1.f);
	...
	//--------------------------------------------------------------------------
	glm::vec3 NightSkyBuilder::ToCartesian(float _theta, float _phi)
	{
		float sinTheta = sin(_theta);
		return glm::vec3(sinTheta*cos(_phi),sinTheta*sin(_phi),cos(_theta));
	}
	//--------------------------------------------------------------------------
	float NightSkyBuilder::PerezFunction(	float A,  
											float B,  
											float C,  
											float D,  
											float E,  
	...
	//-------------------------------------------------------------------------
	glm::vec3 NightSkyBuilder::ComputeSunIntensity(float _sunTheta, float _sunPhi, float _turbidity)
	{
		using namespace glm;
		const float     SunRadius   = 0.018f;
		const float     SunFalloff  = 0.022f;
